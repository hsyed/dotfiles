{ config, pkgs, ... }:
# awww (swaywall wallpaper daemon) for simple, reliable wallpaper management
# Supports rotating wallpapers with wpaperctl-like functionality via custom script
{
  home = {
    # touch the wallpaper dir if needed
    activation.createWallpaperDir = ''
    ...
    # script for rotating wallpapers used by hyprland
    file."${config.home.homeDirectory}/.local/bin/awwwutil" = {
      executable = true;
      text = ''
        #!/usr/bin/env bash
        WALLPAPER_DIR="$HOME/Pictures/wallpapers"
        STATE_FILE="$HOME/.cache/awwwutil_index"
